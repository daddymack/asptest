...
'' Classic ASP Facebook: Version 3.0 01/16/2012
'' by Larry Boeldt
''
...
''
'' 
dim FACEBOOK_APP_ID: FACEBOOK_APP_ID = "174723912729791"
dim FACEBOOK_SECRET: FACEBOOK_SECRET = "a41ad1afedaf0fd99bb7a5a666947c3e"
...
dim FACEBOOK_SCOPE: FACEBOOK_SCOPE = "email,read_stream,user_birthday,publish_stream"
...
function fb_main
      if cookie("uid") = "" then exit function
	...
	' URL Decode the cookie
  	...
  	if (sCookie & "" = "") then 
	    ...
	    if request.querystring("state") = session("state") then
	        ...
	        resp = get_page_contents(token_url)
 
	        token = parse_access_token( resp )
	        expires = parse_expires( resp )
